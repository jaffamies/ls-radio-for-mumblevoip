...
function newPhoneProp()
	deletePhone()
	if not HasModelLoaded("prop_cs_walkie_talkie") then
		RequestModel("prop_cs_walkie_talkie")
		while not HasModelLoaded("prop_cs_walkie_talkie") do
		Citizen.Wait(1)
		end
	end
	phoneProp = CreateObject(GetHashKey("prop_cs_walkie_talkie"), 1.0, 1.0, 1.0, 1, 1, 0)
	local bone = GetPedBoneIndex(PlayerPedId(), 60309)
	AttachEntityToEntity(phoneProp, PlayerPedId(), bone, 0.05, 0.0, 0.025, 270.0, 0.0, 0.0, 1, 1, 0, 0, 2, 1)
  end
